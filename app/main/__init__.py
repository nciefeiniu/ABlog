...
@cache.cached(timeout=60*60*24*30, key_prefix='global', unless=False)
def global_datas():
    """
    ...
    all_tags = Tag.query.order_by(Tag.refer_num.desc()).limit(20).all()
    if all_tags:
    ...
    guestbook = Page.query.filter_by(url_name='guest-book').first()
    if guestbook:
