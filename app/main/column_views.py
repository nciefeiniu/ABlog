...
def get_all_articles_cache(column_id, key):
    ...
    _, column_url = key.split('$#$#')
    articles = Article.query.filter_by(column_id=int(column_id),draft=False).order_by(Article.timestamp.asc()).all()
    items = []
    for i in articles:
        items.append({
            'id': i.id,
...
def set_article_cache(column_id, column_url, id):
    """设置专栏文章缓存"""
    articles = get_all_articles_cache(column_id, 'column$#$#'+column_url)
    ...
    cache_key = '$#$#'.join(['article', column_url, str(id)])
...
def _column(id):
    ...
    articles = get_all_articles_cache(column.id, 'column$#$#' + column.url_name)
...
def article(url, id):
    ...
    article = get_article_cache(column.id, '$#$#'.join(['article', url, str(id)]))
    ...
    articles = get_all_articles_cache(column.id, 'column$#$#' + url)
    data = enum_list(articles)
    ...
    comment_data=get_comments(pid=None,page_id=None,article_id=article['id'],page=page)
    ...
    return render_template('column/article.html', column=column, data=data,max_page=max_page,
                           title=article['title'], article=article,page=page,
